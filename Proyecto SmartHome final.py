...
def registrar_dispositivo():
    ...
    estado = input("Ingrese el estado inicial (encendido o apagado): ")

    while estado not in ['encendido', 'apagado']:
        print("Por favor, ingrese un estado válido (encendido o apagado).")
        estado = input("Ingrese el estado inicial (encendido o apagado): ")

    programacion_automatica = input("¿Desea configurar la programación automática? (si/no): ")
...
def cargar_estado_dispositivos():
    try:
        with open("estado_dispositivos.txt", "r") as file:
            lines = file.readlines()
            for line in lines:
                dispositivo_info = line.strip().split(": ")
                nombre = dispositivo_info[0]
                estado = dispositivo_info[1]
                cerraduras_registradas.append(Cerradura(nombre, estado, ""))
    except FileNotFoundError:
        print("No se encontró el archivo 'estado_dispositivos.txt'")

def guardar_estado_dispositivos():
    try:
        with open("estado_dispositivos.txt", "w") as file:
            for cerradura in cerraduras_registradas:
                file.write(cerradura.nombre + ": " + cerradura.estado + "\n")
        print("Estado de dispositivos guardado con éxito")
    except Exception as e:
        print(f"Error al guardar el estado de dispositivos: {e}")
